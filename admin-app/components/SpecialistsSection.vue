<template>
    <section v-if="visible" class="section">
        <div class="section-header">
            ...
            <button type="button" class="btn" @click="loadSpecialists">
                🔄 {{ $t("common.refresh") }}
            </button>
        </div>
        ...
        <div class="search-bar">
            ...
            <button
                type="button"
                class="btn"
                @click="clearSpecialistsSearch"
                v-if="specialistsSearchQuery"
            >
                ❌ {{ $t("common.clear") }}
            </button>
        </div>

        <div class="list-state" v-if="specialistsLoading">
            {{ $t("adminSpecialists.loading") }}
        </div>
        <div class="list-state error" v-else-if="specialistsError">
            {{ specialistsError }}
        </div>
        ...
        <div class="table" v-else>
            ...
            <div
                ...
                :key="specialist.id"
                class="table-row specialists-table"
            >
                <span>{{ specialist.name }}</span>
                <span>{{ specialist.email }}</span>
                <span>{{ specialist.phone }}</span>
                <span>{{
                    resolveCategoryRates(specialist.categoryRates)
                }}</span>
                <span>{{ specialist.categoryRates.length }}</span>
                <span>{{
                    resolveSpecialistConnections(specialist.connections)
                }}</span>
                <span>{{
                ...
                <span class="row-actions">
                    <button
                        type="button"
                        class="btn"
                        @click="startEditSpecialist(specialist)"
                    >
                        ✏️ {{ $t("common.select") }}
                    </button>
                    <button
                        type="button"
                        class="btn danger"
                        @click="removeSpecialist(specialist.id)"
                    >
                        🗑️ {{ $t("common.delete") }}
                    </button>
                </span>
        ...
        <form
            ref="specialistFormRef"
            class="form"
            v-if="selectedSpecialistId"
            @submit.prevent
        >
            <div class="form-header">
                <h3>{{ $t("adminSpecialists.specialistDetails") }}</h3>
                <span class="form-subtitle" v-if="specialistForm.name">{{
                    specialistForm.name
                }}</span>
            </div>
            ...
            <div class="form-field form-field--full">
                <div class="details-tabs">
                    ...
                    <button
                        ...
                        class="tab"
                        :class="{
                            active: specialistDetailsTab === 'categoryRates',
                        }"
                    ...
                    <button
                        ...
                        class="tab"
                        :class="{
                            active: specialistDetailsTab === 'connections',
                        }"
                    ...
                    <button
                        ...
                        class="tab"
                        :class="{
                            active: specialistDetailsTab === 'notifications',
                        }"
                ...
                <div
                    v-if="specialistDetailsTab === 'general'"
                    class="details-panel"
                >
                    <div class="form-grid">
                        <div class="form-field">
                            ...
                            <input
                                ...
                                type="text"
                                :placeholder="
                                    $t('adminSpecialists.namePlaceholder')
                                "
                            />
                        </div>
                        <div class="form-field">
                            ...
                            <input
                                id="specialist-email"
                                v-model="specialistForm.email"
                                type="email"
                                :placeholder="
                                    $t('adminSpecialists.emailPlaceholder')
                                "
                            />
                        </div>
                        <div class="form-field">
                            ...
                            <input
                                id="specialist-phone"
                                v-model="specialistForm.phone"
                                type="tel"
                                :placeholder="
                                    $t('adminSpecialists.phonePlaceholder')
                                "
                            />
                        </div>
                        <div class="form-field form-field--full">
                            <label for="specialist-bio">{{
                                $t("profile.bio")
                            }}</label>
                            <textarea
                                id="specialist-bio"
                                v-model="specialistForm.bio"
                                :placeholder="$t('profile.bioPlaceholder')"
                                rows="4"
                            ></textarea>
                        </div>
                        <div class="form-field">
                            <label for="specialist-availability">{{
                                $t("profile.availabilityStatus")
                            }}</label>
                            <select
                                id="specialist-availability"
                                v-model="specialistForm.isAvailable"
                            >
                                <option :value="true">
                                    {{ $t("common.available") }}
                                </option>
                ...
                <div
                    v-if="specialistDetailsTab === 'categoryRates'"
                    class="details-panel"
                >
                    <div class="rate-rows">
                        ...
                        <div
                            v-for="(
                                rate, index
                            ) in specialistForm.categoryRates"
                            :key="index"
                            class="rate-row"
                        >
                            ...
                            <button
                                type="button"
                                class="btn"
                                @click="removeCategoryRate(index)"
                            >
                                🗑️ {{ $t("common.remove") }}
                            </button>
                        </div>
                ...
                <div
                    v-else-if="specialistDetailsTab === 'connections'"
                    class="details-panel"
                >
                    ...
                    <div v-else class="connections-manager">
                        <div
                            class="list-state"
                            v-if="specialistConnectionsLoading"
                        >
                            {{ $t("adminSpecialists.loadingConnections") }}
                        </div>
                        <div
                            class="list-state error"
                            v-else-if="specialistConnectionsError"
                        >
                        ...
                        <div class="table" v-else>
                            <div
                                class="table-header specialist-connections-table"
                            >
                        ...
                        <div ref="connectionFormRef" class="form-grid">
                            <div class="form-field">
                                <label for="specialist-connection-type">{{
                                    $t("connections.connectionType")
                                }}</label>
                                <select
                                    id="specialist-connection-type"
                                    v-model="connectionForm.connectionTypeId"
                                >
                                    <option value="">
                                        {{ $t("adminSpecialists.selectType") }}
                                    </option>
                                    <option
                            ...
                            <div class="form-field">
                                <label for="specialist-connection-value">{{
                                ...
                                <input
                                    ...
                                    :placeholder="
                                        $t(
                                            'adminSpecialists.connectionValuePlaceholder',
                                        )
                                    "
                                />
                            </div>
                        ...
                        <div class="form-actions">
                            <button
                                ...
                                @click="addConnection"
                            >
                                ➕ {{ $t("adminSpecialists.addConnection") }}
                            </button>
                            <button
                                type="button"
                                class="btn"
                        ...
                        <p v-if="connectionActionMessage" class="form-message">
                ...
                <div
                    v-else-if="specialistDetailsTab === 'notifications'"
                    class="details-panel"
                >
                    <p v-if="!selectedSpecialistId" class="muted">
                    ...
                    <div v-else class="notifications-manager">
                        <div class="list-state" v-if="notificationsLoading">
                        ...
                        <div v-else class="notifications-list">
                            <div
                                v-for="pref in specialistNotifications"
                                :key="pref.id"
                            ...
                            >
                                ...
                                <label class="toggle-switch">
                                    <input
                                        ...
                                        "
                                        :disabled="
                                            updatingNotificationId === pref.id
                                        "
                                        :aria-label="
                                            formatNotificationType(
                                                pref.notificationType,
            ...
            <div class="form-actions">
                ...
                <button
                    ...
                    :disabled="
                        ...
                        specialistDetailsTab !== 'categoryRates'
                    "
            ...
            <p v-if="specialistActionMessage" class="form-message">
                {{ specialistActionMessage }}
            </p>
        </form>
...
const specialistDetailsTab = ref<
    "general" | "categoryRates" | "connections" | "notifications"
>("general");
        ...
        .map((rate) => {
            const name =
                categories.value.find(
                    (category) => category.id === rate.categoryId,
...
const resolveSpecialistConnections = (connections: SpecialistConnection[]) =>
    connections
        .map(
            (connection) =>
                `${resolveConnectionTypeName(connection.connectionTypeId)}: ${connection.connectionValue}`,
        )
        .join(", ");
...
const loadSpecialists = async () => {
    ...
    try {
        const data = await $fetch<Specialist[]>(
            `${config.public.apiBase}/specialists/search`,
            {
                method: "GET",
                query: {
...
const loadSpecialistConnections = async () => {
    ...
    try {
        ...
        specialistConnections.value = [];
        specialistConnectionsError.value = t(
            "adminSpecialists.failedToLoadConnections",
        );
    } finally {
...
const updateNotificationPreference = async (pref: any) => {
    if (!selectedSpecialistId.value) {
        notificationUpdateMessage.value = t(
            "adminSpecialists.selectToUpdatePreferences",
        );
    ...
    try {
        ...
        notificationUpdateMessage.value = t(
            "adminSpecialists.notificationUpdated",
        );
        notificationUpdateSuccess.value = true;
    } catch (error) {
        notificationUpdateMessage.value = t(
            "adminSpecialists.failedToUpdateNotification",
        );
        notificationUpdateSuccess.value = false;
...
const startEditSpecialist = (specialist: Specialist) => {
    ...
    nextTick(() => {
        if (specialistFormRef.value) {
            specialistFormRef.value.scrollIntoView({
                behavior: "smooth",
                block: "start",
            });
            const firstInput = specialistFormRef.value.querySelector(
                "input",
            ) as HTMLInputElement;
            if (firstInput) {
...
const startEditConnection = (connection: SpecialistConnection) => {
    ...
    nextTick(() => {
        if (connectionFormRef.value) {
            connectionFormRef.value.scrollIntoView({
                behavior: "smooth",
                block: "start",
            });
            const firstInput = connectionFormRef.value.querySelector(
                "select, input",
...
const addSpecialist = async () => {
    ...
    const confirmed = await confirmAction(
        t("adminSpecialists.addSpecialist"),
        t("adminSpecialists.addSpecialistConfirm"),
    );
    if (!confirmed) return;
    try {
...
const updateSpecialist = async () => {
    ...
    const confirmed = await confirmAction(
        t("adminSpecialists.updateSpecialist"),
        t("adminSpecialists.updateSpecialistConfirm"),
    );
    if (!confirmed) return;
    specialistActionMessage.value = "";
    try {
        await $fetch(
            `${config.public.apiBase}/specialists/${selectedSpecialistId.value}`,
            {
                method: "PUT",
                body: {
                    email: specialistForm.value.email,
...
const updateCategoryRates = async () => {
    ...
    const confirmed = await confirmAction(
        t("adminSpecialists.updateCategoryRates"),
        t("adminSpecialists.saveCategoryRatesConfirm"),
    );
    if (!confirmed) return;
    specialistActionMessage.value = "";
    try {
        ...
        specialistActionMessage.value = t(
            "adminSpecialists.failedToUpdateRates",
        );
    }
};
...
const addConnection = async () => {
    ...
    if (
        !connectionForm.value.connectionTypeId ||
        !connectionForm.value.connectionValue
    ) {
        connectionActionMessage.value = t(
            "adminSpecialists.selectConnectionTypeAndValue",
        );
        return;
    }
    const confirmed = await confirmAction(
        t("adminSpecialists.addConnection"),
        t("adminSpecialists.addConnectionConfirm"),
    );
    if (!confirmed) return;
    try {
        await $fetch(
        ...
        connectionActionMessage.value = t(
            "adminSpecialists.failedToAddConnection",
        );
    }
};
...
const updateConnection = async () => {
    ...
    const confirmed = await confirmAction(
        t("adminSpecialists.updateConnection"),
        t("adminSpecialists.updateConnectionConfirm"),
    );
    if (!confirmed) return;
    try {
        await $fetch(
        ...
        connectionActionMessage.value = t(
            "adminSpecialists.failedToUpdateConnection",
        );
    }
};
...
const deleteSelectedConnection = async () => {
    ...
    try {
        ...
        connectionActionMessage.value = t(
            "adminSpecialists.failedToDeleteConnection",
        );
    }
};
