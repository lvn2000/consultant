<template>
    <section v-if="visible" class="section">
        <div class="section-header">
            ...
            <button type="button" class="btn" @click="loadClients">
                🔄 Refresh
            </button>
        </div>
        ...
        <div class="search-bar">
            ...
            <button
                type="button"
                class="btn"
                @click="clearClientsSearch"
                v-if="clientsSearchQuery"
            >
                ❌ Clear
            </button>
        </div>

        <div class="list-state" v-if="clientsLoading">
            {{ $t("adminClients.loading") }}
        </div>
        <div class="list-state error" v-else-if="clientsError">
            {{ clientsError }}
        </div>
        ...
        <div class="table" v-else>
            ...
            <div
                v-for="client in pagedFilteredClients"
                :key="client.id"
                class="table-row clients-table"
            >
        ...
        <div v-if="!clientsLoading && !clientsError" class="pagination">
            <div class="pagination-info">Page {{ clientCurrentPage }}</div>
            <div class="pagination-controls">
                <button
                    type="button"
                    class="btn"
                    :disabled="clientCurrentPage === 1"
                    @click="goToPreviousClientPage"
                >
                    ⬅️ Previous
                </button>
                <button
        ...
        <form ref="clientFormRef" class="form" @submit.prevent>
            <div class="form-header" v-if="selectedClientId">
                ...
                <span class="form-subtitle" v-if="clientForm.name">{{
                    clientForm.name
                }}</span>
            </div>
            ...
            <div class="form-field form-field--full" v-if="selectedClientId">
                <div class="details-tabs">
                    ...
                    <button
                        ...
                        class="tab"
                        :class="{
                            active: clientDetailsTab === 'notifications',
                        }"
                    ...
                    <div class="form-grid">
                        <div class="form-field">
                            ...
                            <input
                                ...
                                type="text"
                                :placeholder="
                                    $t('adminClients.namePlaceholder')
                                "
                            />
                        </div>
                        <div class="form-field">
                            ...
                            <input
                                ...
                                type="email"
                                :placeholder="
                                    $t('adminClients.emailPlaceholder')
                                "
                            />
                        </div>
                        <div class="form-field">
                            ...
                            <input
                                ...
                                type="tel"
                                :placeholder="
                                    $t('adminClients.phonePlaceholder')
                                "
                            />
                        </div>
                    </div>
                ...
                <div
                    v-else-if="clientDetailsTab === 'notifications'"
                    class="details-panel"
                >
                    ...
                    <div v-else class="notifications-manager">
                        ...
                        <div v-else class="notifications-list">
                            <div
                                v-for="pref in clientNotifications"
                                :key="pref.id"
            ...
            <div v-else class="form-grid">
                <div class="form-field">
                    ...
                    <input
                        ...
                        v-model="clientForm.name"
                        type="text"
                        :placeholder="$t('adminClients.namePlaceholder')"
                    />
                </div>
                <div class="form-field">
                    ...
                    <input
                        ...
                        v-model="clientForm.email"
                        type="email"
                        :placeholder="$t('adminClients.emailPlaceholder')"
                    />
                </div>
                <div class="form-field">
                    ...
                    <input
                        ...
                        v-model="clientForm.phone"
                        type="tel"
                        :placeholder="$t('adminClients.phonePlaceholder')"
                    />
                </div>
            </div>
            ...
            <p v-if="clientActionMessage" class="form-message">
                {{ clientActionMessage }}
            </p>
        </form>
...
const isLastClientPage = computed(
...
const startEditClient = (client: Client) => {
    ...
    nextTick(() => {
        if (clientFormRef.value) {
            clientFormRef.value.scrollIntoView({
                behavior: "smooth",
                block: "start",
            });
            const firstInput = clientFormRef.value.querySelector(
                "input",
            ) as HTMLInputElement;
            if (firstInput) {
...
const updateClient = async () => {
    ...
    const confirmed = await confirmAction(
        t("adminClients.updateClient"),
        t("adminClients.updateConfirm"),
    );
    if (!confirmed) return;
    ...
    try {
        clientActionMessage.value = "";
        await $fetch(
            `${config.public.apiBase}/users/${selectedClientId.value}`,
            {
                method: "PUT",
                body: {
                    name: clientForm.value.name,
...
const removeClient = async (id: string) => {
    const confirmed = await confirmAction(
        t("adminClients.deleteClient"),
        t("adminClients.deleteConfirm"),
    );
    if (!confirmed) return;
