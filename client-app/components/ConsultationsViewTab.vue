    ...
    <div class="tab-panel">
        <div class="form" style="margin-bottom: 2rem">
            ...
            <div class="form-grid">
                <div class="form-field">
                    ...
                    <select v-model="filters.status">
                        <option value="">
                            {{ $t("consultations.filter.allStatus") }}
                        </option>
                        <option value="Requested">
                            {{ $t("consultations.filter.requested") }}
                        </option>
                        <option value="Scheduled">
                            {{ $t("consultations.filter.scheduled") }}
                        </option>
                        <option value="InProgress">
                            {{ $t("consultations.filter.inProgress") }}
                        </option>
                        <option value="Completed">
                            {{ $t("consultations.filter.completed") }}
                        </option>
                        <option value="Missed">
                            {{ $t("consultations.filter.missed") }}
                        </option>
                        <option value="Cancelled">
                            {{ $t("consultations.filter.cancelled") }}
                        </option>
                    </select>
                </div>
                <div class="form-field">
                ...
                <div class="form-field">
                    ...
                    <input
                        ...
                        :placeholder="
                            $t('consultations.filter.searchPlaceholder')
                        "
                    />
                </div>
            </div>
            <div class="form-actions">
                <button class="btn btn-secondary" @click="clearFilters">
                    {{ $t("consultations.filter.clearFilters") }}
                </button>
                <button
                    class="btn btn-primary"
                    @click="emit('load-consultations')"
                >
                    {{ $t("consultations.filter.applyFilters") }}
                </button>
            </div>
        </div>
        ...
        <div v-else class="consultation-list">
            <div v-if="filteredConsultations.length === 0" class="empty-state">
                <div v-if="paginatedConsultations.length === 0">
                    {{ $t("consultations.noConsultationsFiltered") }}
                </div>
                ...
                >
                    <div class="consultation-header">
                        ...
                        <div
                            ...
                            :class="consultation.status.toLowerCase()"
                        >
                            {{ consultation.status }}
                        </div>
                    </div>
                    <div class="consultation-details">
                        <div>
                            {{ $t("consultations.specialistLabel")
                            }}{{ getSpecialistName(consultation.specialistId) }}
                        </div>
                        <div>
                            {{ $t("consultations.durationLabel")
                            }}{{
                                $t("consultations.durationMinutes", {
                                    n: consultation.duration,
                                })
                            }}
                        </div>
                        <div>
                            {{ $t("consultations.priceLabel")
                            }}{{
                ...
                <span class="pagination-info">
                    {{
                        $t("common.pagination.pageWithTotal", {
                            ...
                            items: paginationInfo.totalCount,
                        })
                    }}
                </span>
                <button
...
onMounted(async () => {
    try {
        ...
        specialistMap.value = specialists.value.reduce((acc, spec) => {
            acc[spec.id] = spec.name;
            return acc;
        }, {});
    } catch (e) {
        console.error("Failed to load specialists for consultation view:", e);
    }
});

// Helper to get specialist name from ID
function getSpecialistName(specialistId: string): string {
    return specialistMap.value[specialistId] || specialistId;
}
...
const filteredConsultations = computed(() => {
    ...
    if (filters.value.fromDate) {
        ...
        filtered = filtered.filter(
            (c) => new Date(c.createdAt || c.date) >= fromDate,
        );
    }
    ...
    if (filters.value.toDate) {
        ...
        filtered = filtered.filter(
            (c) => new Date(c.createdAt || c.date) <= toDate,
        );
    }
